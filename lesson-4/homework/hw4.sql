...
-- 11. Narxi eng qimmat 10 mahsulot
SELECT TOP 10 * FROM Products ORDER BY Price DESC;

-- 12. FirstName NULL bo‘lsa, LastName ni chiqarish
SELECT EmployeeID, COALESCE(FirstName, LastName) AS Name 
FROM Employees;

-- 13. Takrorlanmagan Category va Price qiymatlari
SELECT DISTINCT Category, Price FROM Products;

-- 14. Yoshi 30-40 oralig‘ida bo‘lgan yoki Marketing bo‘limidagi xodimlar
SELECT * FROM Employees 
WHERE (Age BETWEEN 30 AND 40) OR DepartmentName = 'Marketing';

-- 15. Maoshi bo‘yicha 11–20-o‘rindagi xodimlar
SELECT * FROM Employees 
ORDER BY Salary DESC
OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY;

-- 16. Narxi 1000 dan kichik va Stock 50 dan katta mahsulotlar
SELECT * FROM Products 
WHERE Price <= 1000 AND StockQuantity > 50 
ORDER BY StockQuantity ASC;

-- 17. Nomida "e" harfi qatnashgan mahsulotlar
SELECT * FROM Products WHERE ProductName LIKE '%e%';

-- 18. HR, IT yoki Finance bo‘limidagi xodimlar
SELECT * FROM Employees 
WHERE DepartmentName IN ('HR','IT','Finance');

-- 19. Mijozlar: shahar bo‘yicha o‘sish, postal code bo‘yicha kamayish tartibida
SELECT * FROM Customers 
ORDER BY City ASC, PostalCode DESC;

---qiyin

-- 20. Eng ko‘p sotilgan 5 mahsulot
...
-- 21. FirstName + LastName birlashtirib FullName qilib chiqarish
SELECT EmployeeID, (FirstName + ' ' + LastName) AS FullName 
FROM Employees;

-- 22. Narxi 50 dan katta, takrorlanmagan Category va ProductName
SELECT DISTINCT Category, ProductName, Price 
FROM Products
WHERE Price > 50;

-- 23. O‘rtacha narxning 10% dan kichik mahsulotlar
SELECT * FROM Products
WHERE Price < (SELECT AVG(Price) * 0.1 FROM Products);

-- 24. Yoshi 30 dan kichik va HR yoki IT bo‘limidagi xodimlar
SELECT * FROM Employees
WHERE Age < 30 AND DepartmentName IN ('HR','IT');

-- 25. Email manzili Gmail bo‘lgan mijozlar
SELECT * FROM Customers WHERE Email LIKE '%@gmail.com%';

-- 26. Maoshi Sales bo‘limidagi hamma xodimlardan ko‘p bo‘lgan xodimlar
SELECT * FROM Employees
WHERE Salary > ALL (
    SELECT Salary FROM Employees WHERE DepartmentName = 'Sales'
);

-- 27. Oxirgi 180 kun ichida berilgan buyurtmalar
SELECT * FROM Orders
