...
---oson---
-- 1. 50 000 dan katta maosh oladigan xodimlar
SELECT e.Name AS EmployeeName, e.Salary, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID
WHERE e.Salary > 50000;

-- 2. 2023-yilda berilgan buyurtmalar
SELECT c.FirstName, c.LastName, o.OrderDate
FROM Customers c
JOIN Orders o ON c.CustomerID = o.CustomerID
WHERE YEAR(o.OrderDate) = 2023;

-- 3. Xodimlar va bo‘limlari (hammasi chiqsin)
SELECT e.Name AS EmployeeName, d.DepartmentName
FROM Employees e
LEFT JOIN Departments d ON e.DepartmentID = d.DepartmentID;

-- 4. Yetkazib beruvchilar va mahsulotlari
SELECT s.SupplierName, p.ProductName
FROM Suppliers s
LEFT JOIN Products p ON s.SupplierID = p.SupplierID;

-- 5. Buyurtmalar va to‘lovlari
SELECT o.OrderID, o.OrderDate, p.PaymentDate, p.Amount
FROM Orders o
FULL OUTER JOIN Payments p ON o.OrderID = p.OrderID;

-- 6. Har bir xodim va uning boshlig‘i
...
-- 8. 3 tadan ko‘p mahsulot buyurtma qilgan mijozlar
...
---o'rta---
-- 1. 5 tadan ko‘p xodimi bor bo‘limlar
...
-- 2. Sotilmagan mahsulotlar
SELECT p.ProductID, p.ProductName
FROM Products p
LEFT JOIN Sales s ON p.ProductID = s.ProductID
WHERE s.ProductID IS NULL;

-- 3. Hech bo‘lmaganda 1 ta buyurtma qilgan mijozlar
SELECT c.FirstName, c.LastName, COUNT(o.OrderID) AS TotalOrders
FROM Customers c
JOIN Orders o ON c.CustomerID = o.CustomerID
GROUP BY c.FirstName, c.LastName;

-- 4. Faqat mos bo‘lgan xodimlar va bo‘limlari
SELECT e.Name AS EmployeeName, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID;

-- 5. Bir xil boshliq ostida ishlaydigan xodim juftliklari
SELECT e1.Name AS Employee1, e2.Name AS Employee2, e1.ManagerID
FROM Employees e1
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID
WHERE e1.EmployeeID < e2.EmployeeID;

-- 6. 2022-yilda berilgan buyurtmalar
SELECT o.OrderID, o.OrderDate, c.FirstName, c.LastName
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
WHERE YEAR(o.OrderDate) = 2022;

-- 7. "Sales" bo‘limida ishlaydigan va oyligi 60000 dan yuqori xodimlar
SELECT e.Name, e.Salary, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID
WHERE d.DepartmentName = 'Sales' AND e.Salary > 60000;

-- 8. To‘lov qilingan buyurtmalar
SELECT o.OrderID, o.OrderDate, p.PaymentDate, p.Amount
FROM Orders o
JOIN Payments p ON o.OrderID = p.OrderID;

-- 9. Buyurtma qilinmagan mahsulotlar
SELECT p.ProductID, p.ProductName
FROM Products p
...
---qiyin---
-- 1. O‘z bo‘limi bo‘yicha o‘rtacha oylikdan yuqori maosh oladigan xodimlar
SELECT e.Name, e.Salary, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID
WHERE e.Salary > (
...
-- 2. 2020-yilgacha berilgan, lekin to‘lanmagan buyurtmalar
SELECT o.OrderID, o.OrderDate
FROM Orders o
LEFT JOIN Payments p ON o.OrderID = p.OrderID
WHERE o.OrderDate < '2020-01-01' AND p.PaymentID IS NULL;

-- 3. Kategoriyasi yo‘q mahsulotlar
SELECT p.ProductID, p.ProductName
FROM Products p
LEFT JOIN Categories c ON p.CategoryID = c.CategoryID
WHERE c.CategoryID IS NULL;

-- 4. Bir xil boshliq ostida va 60000 dan yuqori oylik oladigan xodimlar juftligi
SELECT e1.Name AS Employee1, e2.Name AS Employee2, m.Name AS ManagerName
FROM Employees e1
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID
JOIN Employees m ON e1.ManagerID = m.EmployeeID
WHERE e1.EmployeeID < e2.EmployeeID
  AND e1.Salary > 60000
  AND e2.Salary > 60000;

-- 5. Nomi "M" harfi bilan boshlanadigan bo‘limlardagi xodimlar
SELECT e.Name, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID
WHERE d.DepartmentName LIKE 'M%';

-- 6. Summasi 500 dan yuqori savdolar
SELECT s.SaleID, p.ProductName, s.SaleAmount
FROM Sales s
JOIN Products p ON s.ProductID = p.ProductID
WHERE s.SaleAmount > 500;

-- 7. "Math 101" kursiga yozilmagan talabalar
...
-- 8. To‘lov tafsiloti yo‘q buyurtmalar
SELECT o.OrderID, o.OrderDate
FROM Orders o
LEFT JOIN Payments p ON o.OrderID = p.OrderID
WHERE p.PaymentID IS NULL;

-- 9. "Electronics" va "Furniture" kategoriyasidagi mahsulotlar
