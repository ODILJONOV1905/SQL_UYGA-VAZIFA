...
-- 3. Products va Suppliers (faqat real supply bo‘yicha)
...
-- 6. Products × Orders (ProductID mos bo‘lsa)
SELECT p.ProductName, o.OrderID
FROM Orders o
JOIN Products p ON o.ProductID = p.ProductID;

-- 7. Employees × Departments (DepartmentID mos bo‘lsa)
...
-- 9. Payments × Orders (mos kelgan orderlar)
SELECT o.OrderID, p.PaymentID, p.Amount
FROM Orders o
JOIN Payments p ON o.OrderID = p.OrderID;

-- 10. Orders × Products (narxi > 100)
...
-- 1. Employees × Departments (mos bo‘lmagan kombinatsiyalar)
SELECT e.Name, d.DepartmentName
FROM Employees e
CROSS JOIN Departments d
WHERE e.DepartmentID <> d.DepartmentID;

-- 2. Orders × Products (miqdor > zaxira)
SELECT o.OrderID, p.ProductName, o.Quantity, p.StockQuantity
FROM Orders o
JOIN Products p ON o.ProductID = p.ProductID
WHERE o.Quantity > p.StockQuantity;

-- 3. Customers × Sales (summasi >= 500)
SELECT c.FirstName + ' ' + c.LastName AS CustomerName, s.ProductID
FROM Sales s
JOIN Customers c ON s.CustomerID = c.CustomerID
WHERE s.Amount >= 500;

-- 4. Courses × Enrollments × Students
SELECT s.StudentName, c.CourseName
FROM Enrollments e
JOIN Students s ON e.StudentID = s.StudentID
JOIN Courses c ON e.CourseID = c.CourseID;

-- 5. Products × Suppliers (Tech bo‘lgan)
SELECT p.ProductName, s.SupplierName
FROM Products p
JOIN Suppliers s ON p.SupplierID = s.SupplierID
...
-- 7. Employees × Departments (har bir xodimning departmenti)
SELECT e.Name, d.DepartmentName
FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID;
...
-- 9. Sales × Customers (USA)
SELECT s.SaleID, c.FirstName + ' ' + c.LastName AS CustomerName
FROM Sales s
JOIN Customers c ON s.CustomerID = c.CustomerID
WHERE c.Country = 'USA';

-- 10. Orders × Customers (Germany va total > 100)
...
---qiyin---
-- 1. Employees: boshqa departmentdagi juftliklar
SELECT e1.Name AS Employee1, e2.Name AS Employee2
FROM Employees e1
JOIN Employees e2 ON e1.EmployeeID < e2.EmployeeID
WHERE e1.DepartmentID <> e2.DepartmentID;

-- 2. Payments ≠ Orders.Price × Quantity
...
-- 4. Employees: menejer bo‘lib, o‘zidan kam maosh olmagan
...
-- 5. Orders × Customers (to‘lov qilinmagan buyurtmalar)
SELECT c.FirstName + ' ' + c.LastName AS CustomerName, o.OrderID
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
