...
GROUP BY 
    CASE 
        ...
        ELSE '50+'
    END;
SELECT CustomerID, MAX(OrderDate) AS OxirgiBuyurtma
FROM Orders
GROUP BY CustomerID;
...
SELECT TOP 3 CustomerID, COUNT(*) AS BuyurtmalarSoni
FROM Orders
GROUP BY CustomerID
ORDER BY BuyurtmalarSoni DESC;
SELECT MAX(Salary) - MIN(Salary) AS OylikFarqi
FROM Employees;
SELECT *
FROM Employees
ORDER BY Salary DESC;
SELECT TOP 1 YEAR(SaleDate) AS SotuvYili, SUM(SaleAmount) AS UmumiySotuv
FROM Sales
GROUP BY YEAR(SaleDate)
ORDER BY UmumiySotuv DESC;
SELECT p.ProductName, SUM(s.SaleAmount) AS UmumiySotuv
FROM Sales s
JOIN Products p ON s.ProductID = p.ProductID
GROUP BY p.ProductName;
SELECT CustomerID, COUNT(DISTINCT ProductID) AS MahsulotlarSoni
FROM Sales
GROUP BY CustomerID
HAVING COUNT(DISTINCT ProductID) > 2;
SELECT DepartmentName, AVG(Age) AS OrtachaYosh
FROM Employees
GROUP BY DepartmentName;
