...
FROM Products
WHERE ProductCategory = 'Laptop' AND Price BETWEEN 100 AND 500;
...
SELECT C.FirstName + ' ' + C.LastName AS CustomerName, O.OrderID, O.OrderDate, P.ProductName
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID
JOIN LastOrder L ON O.CustomerID = L.CustomerID AND O.OrderDate = L.MostRecentOrder
JOIN Products P ON O.ProductID = P.ProductID;

SELECT D.DepartmentName, MAX(E.Salary) AS MaxSalary, MIN(E.Salary) AS MinSalary
FROM Employees E
JOIN Departments D ON E.DepartmentID = D.DepartmentID
GROUP BY D.DepartmentName;

WITH ProductSales AS (
    SELECT O.ProductID, O.CustomerID, SUM(O.Quantity) AS TotalSold
    FROM Orders O
    WHERE YEAR(O.OrderDate) = 2023
    GROUP BY O.ProductID, O.CustomerID
)
SELECT P.ProductName, C.FirstName + ' ' + C.LastName AS CustomerName, PS.TotalSold
FROM ProductSales PS
JOIN Products P ON PS.ProductID = P.ProductID
JOIN Customers C ON PS.CustomerID = C.CustomerID
WHERE PS.TotalSold = (
